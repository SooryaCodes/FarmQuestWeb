...
interface StatusItem {
  time: string;
  status: string;
  state: "completed" | "not-planted" | "not-updated" | "in-progress" | "delayed";
  icon?: string;
  warning?: boolean;
  details?: string;
}
      ...
      <header className="sticky top-0 z-50 flex items-center p-3 md:p-4 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-5xl mx-auto w-full flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h1 className="text-lg md:text-xl font-semibold tracking-tight text-[#0F6435]">Growth Status</h1>
          </div>
          <Button 
            variant="ghost" 
            size="icon" 
            ...
            <div key={index} className="flex mb-12 md:mb-16 group">
              ...
              <div className="flex flex-col items-center mr-5 md:mr-8">
                <div className={cn(
                  ...
                  item.state === "completed" ? "bg-emerald-50 border-[#77AD3F]" : 
                  item.state === "in-progress" ? "bg-blue-50 border-blue-400 animate-pulse" :
                  item.state === "delayed" ? "bg-amber-50 border-amber-400" :
                  "bg-gray-50 border-gray-200"
                )}>
                  {item.state === "completed" && (
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-[#77AD3F] animate-in fade-in-50 duration-300">
                      <path d="M20 6 9 17l-5-5"/>
                    </svg>
                  )}
                  {item.state === "in-progress" && (
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-blue-500 animate-spin-slow">
                      <path d="M12 2v4m0 12v4M4.93 4.93l2.83 2.83m8.48 8.48 2.83 2.83M2 12h4m12 0h4M4.93 19.07l2.83-2.83m8.48-8.48 2.83-2.83"/>
                    </svg>
                  )}
                  {item.state === "delayed" && (
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-amber-500 animate-pulse">
                      <circle cx="12" cy="12" r="10"/>
                      <polyline points="12 6 12 12 16 14"/>
                    </svg>
                  )}
                  {(item.state === "not-planted" || item.state === "not-updated") && (
                ...
                {index < statusItems.length - 1 && (
                  <div className={cn(
                    ...
                    item.state === "completed" ? "bg-gradient-to-b from-[#77AD3F] to-[#77AD3F]/30" : 
                    item.state === "in-progress" ? "bg-gradient-to-b from-blue-400 to-blue-400/30" :
                    item.state === "delayed" ? "bg-gradient-to-b from-amber-400 to-amber-400/30" :
                    "bg-gray-200"
                  )} />
                )}
              </div>
              ...
              <div className="flex-1 bg-white rounded-xl p-4 md:p-6 shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100 group-hover:translate-x-1">
                <div className="flex items-center mb-3">
                  <span className="text-gray-600 font-medium bg-gray-100 px-3 py-1 rounded-full text-sm group-hover:bg-gray-200 transition-colors duration-300">{item.time}</span>
                  <div className={cn(
                    "w-2 h-2 rounded-full ml-3 group-hover:w-3 group-hover:h-3 transition-all duration-300",
                    item.state === "completed" ? "bg-[#77AD3F]" :
                    item.state === "in-progress" ? "bg-blue-400" :
                    item.state === "delayed" ? "bg-amber-400" :
                    "bg-gray-400"
                  )} />
                </div>
                <div className="flex items-center mb-4">
                  <span className={cn(
                    ...
                    item.state === "completed" ? "text-[#0F6435]" : 
                    item.state === "in-progress" ? "text-blue-700" :
                    item.state === "delayed" ? "text-amber-700" :
                    "text-gray-800"
                  )}>{item.status}</span>
                  {item.warning && (
                ...
                {item.details && (
                  <p className="text-gray-600 mb-4 text-sm md:text-base group-hover:text-gray-800 transition-colors duration-300">{item.details}</p>
                )}
                ...
                <div className="flex items-center">
                  {item.icon && (
                    <div className="w-12 h-12 mr-4 flex items-center justify-center bg-gray-50 rounded-full group-hover:bg-gray-100 transition-colors duration-300 group-hover:scale-110">
                      {item.state === "completed" ? (
                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[#77AD3F] group-hover:rotate-6 transition-transform duration-300">
                          <path d="M5 12h4l2-8 2 8h4" />
                          <path d="M3 20h18" />
                          <path d="M12 15v5" />
                        </svg>
                      ) : item.state === "in-progress" ? (
                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-blue-500 group-hover:rotate-6 transition-transform duration-300">
                          <path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z" />
                          <circle cx="12" cy="12" r="3" />
                        </svg>
                      ) : item.state === "delayed" ? (
                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-amber-500 group-hover:rotate-6 transition-transform duration-300">
                          <path d="M19 4H5a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2Z" />
                          <path d="M12 14v-4" />
                          <path d="M12 14h.01" />
                        </svg>
                      ) : (
                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400 group-hover:rotate-6 transition-transform duration-300">
                  ...
                  <div className={cn(
                    ...
                    {item.state === "completed" ? "Completed" : 
                     ...
                     item.state === "delayed" ? "Delayed" :
                     item.state === "not-planted" ? "Not Planted" : "Not Updated"}
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="p-4 md:p-6 max-w-5xl mx-auto w-full mb-8">
        <Card className="bg-gradient-to-r from-[#77AD3F] to-[#0F6435] text-white p-6 md:p-8 rounded-xl overflow-hidden relative shadow-xl hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-1">
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <h2 className="text-3xl md:text-4xl font-bold mb-4 tracking-tight">Keep Nourishing!</h2>
            <p className="text-sm md:text-base opacity-90 leading-relaxed max-w-md mb-6">
              Keep Growing Healthy, Thriving Plants With Us And Enjoy Lush Greenery, Vibrant Blooms, And A Flourishing Garden Every Season!
            </p>
