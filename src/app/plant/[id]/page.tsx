...
const dummyPlant: PlantType = {
  id: 1,
  name: "Tomato Plant",
  description: "A versatile plant that produces juicy red fruits. Perfect for salads and cooking. Tomatoes are rich in vitamins and antioxidants, making them a healthy addition to any diet. They thrive in warm conditions with plenty of sunlight.",
  full_image_url: "/images/plant-1.png",
  cropped_image_url: "/images/plant-1.png",
  basic_needs: ["Full sunlight", "Regular watering", "Well-drained soil", "Support structure"],
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        ...
        <div>
          ...
          <div className="flex items-center mb-4">
            <span className="text-[#77AD3F] text-xl font-medium">★ {plant.rating}</span>
            <span className="text-gray-500 text-sm ml-2">(200 Reviews)</span>
          </div>
          
          {/* Description */}
          <div className="mb-6">
            <p className="text-gray-700 leading-relaxed mb-2">{plant.description}</p>
            <button className="text-[#0F6435] font-medium hover:text-[#77AD3F]">Read more</button>
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              {plant.basic_needs.map((need, index) => (
                <div key={index} className="flex items-center bg-gray-50 rounded-lg p-4 border border-gray-100">
                  <div className={`${index % 2 === 0 ? 'bg-gradient-to-r from-[#77AD3F] to-[#0F6435]' : 'bg-yellow-400'} p-3 rounded-md mr-3`}>
                    {index % 2 === 0 ? (
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-800">{need.split(' ')[0]}</p>
                    <p className="text-sm text-gray-600">{need.split(' ').slice(1).join(' ')}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {Array(4).fill(0).map((_, index) => (
            <Link key={index} href={`/plant/${index + 2}`} className="bg-gray-50 rounded-xl p-4 transition-all hover:shadow-md">
